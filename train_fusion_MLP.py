...
class Config:
    ...
    tabpfn_features_path = "D:\\Graduate\\Features\\tabpfn_features_wholedata.csv"
    ...
    learning_rate = 1e-2
    num_epochs = 500
    patience = 60          # 早停耐心值（基于训练集）
    
    # 数据划分 - 只分训练集和测试集
    test_size = 0.3
    random_seed = 42 #4的数据很好 #9 #42 0.733  4 256 16 5e-3 0.779
...
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(self, config):
        ...
        #     # 第一层：256 → 128
        #     nn.Linear(config.hidden_dim, 128),  # 稍微降维
        #     nn.BatchNorm1d(128),                # BatchNorm提升训练稳定性
        #     nn.ReLU(),
        #     nn.Dropout(config.dropout),   # 稍高的Dropout（0.36）
        ...
        self.classifier = nn.Sequential(
            # 第一层：
            nn.Linear(config.hidden_dim, 256),  # 稍微降维
            nn.BatchNorm1d(256),                # BatchNorm提升训练稳定性
            nn.GELU(),
            nn.Dropout(config.dropout),  
    ...
    def forward(self, clinical_features, image_features):
        ...
        attended_features, attention_weights = self.cross_attention(
            query=clinical_seq,#image_seq
            key=image_seq,#clinical_seq
            value=image_seq,
            need_weights=True  # 返回注意力权重用于可视化
        )
        
        # 移除序列维度 (batch, hidden_dim)
        attended_features = attended_features.squeeze(1)
        
        # 3. 注意力后处理
        fused_features = self.post_attention(attended_features) + attended_features  # 残差连接
...
class DualCrossAttentionFusion(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 5. 分类头
        self.classifier = nn.Sequential(
            nn.Linear(config.hidden_dim, config.hidden_dim//2),  # 稍微降维
    ...
    def forward(self, clinical_features, image_features):
        ...
        # 门控融合
        fused_features = gate * fused_clinical + (1 - gate) * fused_image
...
def train_model(config):
    ...
    # 1. 加载数据
    train_loader, test_loader, data_info = load_and_prepare_data(config)
    
    # 2. 初始化模型
    print("\n🔧 初始化模型...")
    #model = CrossAttentionFusion(config)
    model = DualCrossAttentionFusion(config)
    ...
    for epoch in range(config.num_epochs):
        ...
        if train_acc > best_train_acc or (train_acc == best_train_acc and train_loss < best_train_loss):
            ...
            if patience_counter >= config.patience:
                print(f"🛑 早停触发，训练指标 {config.patience} 个epoch未提升")
                break
